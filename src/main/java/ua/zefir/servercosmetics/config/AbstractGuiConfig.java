...
public abstract class AbstractGuiConfig {
    ...
    private void loadGuiSize(int guiRows) {
        if (guiRows < 1 || guiRows > 6) {
            ModInit.LOGGER.warn("Invalid guiRows value '{}' in {}. Must be between 1 and 6. Defaulting to 6.", guiRows, this.configFilePath.getFileName());
            guiRows = 6;
        }
    ...
    public ConfigManager.NavigationButton getButtonConfig(String buttonKey) {
        ...
        if (button == null) {
            ModInit.LOGGER.warn("Requested non-existent button config: '{}' from {}", buttonKey, this.configFilePath.getFileName());
