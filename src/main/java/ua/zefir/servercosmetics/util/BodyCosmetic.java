...
public class BodyCosmetic implements ICosmetic {
    ...
    public BodyCosmetic(ServerPlayerEntity player, ItemType itemType) {
        if (useArmorStand) {
            ...
            this.bodyCosmeticsModel = new DisplayEntity.ItemDisplayEntity(EntityType.ITEM_DISPLAY, player.getEntityWorld());
        }
        this.player = player;
    ...
    public void init() {
        ...
        if (cosmeticItemStack != ItemStack.EMPTY) {
            player.getEntityWorld().getChunkManager().sendToNearbyPlayers(player,
