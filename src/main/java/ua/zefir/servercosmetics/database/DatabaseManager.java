...
public class DatabaseManager {
    ...
    public static void setCosmetic(ServerPlayerEntity player, ItemType type, ItemStack itemStack) {
        try {
            ...
            if (cosmeticId == null) {
                ModInit.LOGGER.warn("Attempted to set a cosmetic with an ItemStack lacking a 'cosmeticItemId' for player {}", player.getUuidAsString());
                if (existingEntry != null) {
                    cosmeticDao.delete(existingEntry);
                }
                return;
            ...
            ModInit.LOGGER.error("Error saving cosmetic data for player {} and type {}", player.getUuidAsString(), type, e);
    ...
    public static ItemStack getCosmeticItemStack(ServerPlayerEntity player, ItemType type) {
        try {
            ...
            if (cosmeticDefinition == null) {
                ModInit.LOGGER.warn("Player {} has cosmetic '{}' equipped, but it's no longer registered.", player.getName().getString(), cosmeticData.getCosmeticId());
                return ItemStack.EMPTY;
            }
            ...
            ModInit.LOGGER.error("Error loading cosmetic data for player {} and type {}", player.getUuidAsString(), type, e);
