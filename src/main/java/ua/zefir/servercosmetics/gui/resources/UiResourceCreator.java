package ua.zefir.servercosmetics.gui.resources;
...
public class UiResourceCreator {
  public static final Style STYLE = Style.EMPTY.withColor(0xFFFFFF).withFont(ModInit.id("gui"));
  ...
  public static void generateAssets(BiConsumer<String, byte[]> assetWriter) {
...

    {
      ...
      SPACES.char2IntEntrySet().stream()
          .sorted(Comparator.comparing(Char2IntMap.Entry::getCharKey))
          .forEach(
              (c) -> advances.addProperty(Character.toString(c.getCharKey()), c.getIntValue()));
      spaces.add("advances", advances);
      providers.add(spaces);
    }

    FONT_TEXTURES.forEach(
        (entry) -> {
  ...
  public static void setup() {
    ...
    PolymerResourcePackUtils.RESOURCE_PACK_CREATION_EVENT.register(
        (b) -> UiResourceCreator.generateAssets(b::addData));
  }
